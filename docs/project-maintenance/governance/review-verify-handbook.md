...
- closeout / gate / backfill 变更
  ...
  - closeout `test_gate` 会运行 closeout gate、folder logic、path governance、semantic governance、agents adapter contract 回归测试、Repo Analysis contract check、本地 npm deploy package 的 `npm pack --dry-run --json` packlist 检查、临时 `.tgz` tarball smoke，以及 `adapter_deploy.py` / `harness_deploy.py` 的 `agents` deploy verify。
