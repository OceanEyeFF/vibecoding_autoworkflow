...
- 新增或移动 `docs/` 正文文档必须接入 `docs/book.md` spine：先挂到最近章节 `README.md` 或由 `docs/book.md` 直接链接，再确认从 `docs/book.md` 沿相对 markdown 链接可达
...
- 新增、移动、拆分或删除 `docs/` 正文：`path_governance_check.py`，确认新正文可从 `docs/book.md` spine 到达且最近章节入口已更新
...
docs restructuring closeout 必须记录 reader-coherence evidence，至少说明 `docs/book.md`、最近章节入口和移动后的正文之间的阅读路径，以及删除/迁移后的旧入口如何处理。若当前任务无法完成 reader-coherence 验证，必须在 closeout 中写明 explicit deferral：延期原因、剩余入口风险、后续验证命令和接管位置。
