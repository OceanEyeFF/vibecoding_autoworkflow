...
- 已退役的兼容入口路径是否以 inline code 或普通文本形式回流到根入口文档
...
运行治理脚本时默认保留 `PYTHONDONTWRITEBYTECODE=1`，避免检查过程自己在源码树中生成 `.pytest_cache`、`__pycache__`、`.pyc` 或 `.pyo`。
...
19. 关键承接关系是否仍存在：
   ...
   - `docs/harness/README.md -> docs/harness/{foundations,artifact,workflow-families}/README.md`
   - `product/harness/README.md -> docs/harness/README.md`
   - `docs/harness/artifact/worktrack/README.md -> worktrack contract / queue / gate evidence`
20. foundations 权威文档是否出现同名前缀 shadow 文件
21. 关键入口文档是否重新出现已退役的“预留位 / 占位”口径
22. 根兼容入口是否仍避免引用已退役的 adjacent-systems 子页
