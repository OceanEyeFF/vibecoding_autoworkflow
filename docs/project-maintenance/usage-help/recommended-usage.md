...
安装完成后，运行 `set-harness-goal-skill` 初始化 `.aw/`：
...
在 Claude Code 中直接提出第一个任务：

```txt
我想先做 [第一个功能/任务]，范围是 [具体路径]，不做 [排除事项]
```
...
| 变更类型 | 条件 | 操作 |
|---------|------|------|
| 追加新功能（在现有目标内） | 新需求在现有 Goal Charter 范围内 | 直接 `/harness-skill` 提交请求 |
