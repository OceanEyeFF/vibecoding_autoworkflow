...
`docs/` 一级文档域：`book.md`、`project-maintenance/`、`harness/`、`analysis/`、`ideas/`、`archive/`。`book.md` 是 docs 书式章节 spine，只承接阅读顺序、章节边界和新增文档放置判断；长期规则正文仍进入最近的 `project-maintenance/` 或 `harness/` 承接文档。`analysis/` 承接尚未升格为主线规则的分析材料，`ideas/` 承接未承诺实施的想法草稿，`archive/` 承接历史保留与退役上下文。`product/` 一级源码根：`harness/`；`product/` 下允许 `.aw_template/`（repo-local execution template layer，非 artifact 模板长期 owner）。
