...
| Path | Current role | Removal classification | Rationale |
|---|---|---|---|
| `toolchain/scripts/deploy/bin/aw-installer.js` | Primary Node bin. Owns help/version, `agents` package/local lifecycle, selected local agents unsupported-variant failures, `claude` package/local lifecycle, `agents` GitHub-source JSON dry-run, TUI shell, environment filtering and Python fallback. | retain | This is the target owner for deploy runtime. Through Slice H, Node-owned behavior covers package/local agents `diagnose`, `check_paths_exist`, `verify`, clean and non-clean-conflict `install`, recognized same-backend managed-install `prune --all`, `update --backend agents` dry-run, TUI dry-run display, `update --backend agents --yes` composition, and selected local invalid variants such as prune without `--all` and update `--json --yes`. Slice I-claude-readonly covers package/local Claude diagnose human/JSON, `--claude-root`, check_paths_exist, verify and update dry-run human/JSON. Slice I-claude-mutating covers package/local Claude install, prune --all and update --yes with full Harness skill payload, `.claude/skills/<skill_id>` naming, frontmatter transform, same-backend managed legacy cleanup, strict verify and backend-aware recovery hints. Slice J-github-json covers explicit `agents` GitHub-source JSON dry-run with repo/ref/SHA validation, safe ZIP extraction, source contract validation, temp cleanup and source/target separation. GitHub-source human dry-run/apply and other unmigrated deploy modes still fall back to Python/reference. |
