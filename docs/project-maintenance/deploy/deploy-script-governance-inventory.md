...
已验证进度：Slice A 已把 package/local-source `aw-installer check_paths_exist --backend agents` 收敛为 Node-owned 只读路径；Slice B 已把 package/local-source `aw-installer verify --backend agents` 收敛为 Node-owned 只读路径；Slice C 已把 package/local-source clean-target `aw-installer install --backend agents` 收敛为 Node-owned 写入路径；Slice D 已把 package/local-source `aw-installer prune --all --backend agents` 收敛为 Node-owned 删除路径；Slice E 已把 package/local-source `aw-installer update --backend agents --yes` 收敛为 Node-owned composition path；Slice G 已把 package/local-source human-readable `aw-installer update --backend agents` dry-run 与 TUI dry-run 展示收敛为 Node-owned plan rendering；Slice H 已把 package/local-source `diagnose --backend agents` human/JSON root override、non-clean target standalone `install --backend agents` 的 planned-path 冲突阻断和 `prune --backend agents` 缺少 `--all`、`update --backend agents --json --yes` 等本地 agents 无效组合收敛为 Node-owned no-Python 路径；Slice I-claude-readonly 已把 package/local-source `claude` backend 的 diagnose human/JSON、`--claude-root` override、check_paths_exist、verify 和 update dry-run human/JSON 收敛为 Node-owned no-Python 只读/预检路径；Slice J-github-json 已把显式 `agents` GitHub-source JSON dry-run（`update --backend agents --json --source github ...`）收敛为 Node-owned no-Python 远程 source 预检路径。已完成切片均保留 Python reference parity 或 failing-Python sentinel 证据；Slice C/Slice H 覆盖 `install --backend agents` 的 clean-target 写入和 non-clean planned-path 冲突阻断，路径冲突时必须在写入前失败；无关用户内容不属于 planned-path conflict；Slice D 只删除 current-backend recognized managed installs，并保留 foreign、unrecognized、invalid-marker 和用户内容；Slice E 只组合已验证的 `prune --all -> check_paths_exist -> install -> verify`，保留 blocking preflight、failure short-circuit、post-apply strict verify 和 recovery hint；Slice G 只覆盖 package/local `agents` dry-run rendering，不扩展 apply 语义；Slice I-claude-readonly 不覆盖 Claude install、prune --all 或 update --yes；Slice J-github-json 不覆盖 GitHub-source human dry-run或 update --yes apply。不碰 release metadata，不改变 package version，不删除 Python reference。

下一步建议：把后续工作排成 runtime no-Python 迁移切片，而不是一次性删除 Python。Claude mutating lifecycle（install、prune --all、update --yes）和 GitHub-source human dry-run/apply 仍需独立切片；package packlist removal、Python runtime file removal、release metadata 或实际发布属于 release/package boundary，都需要新的显式审批和独立 gate。
...
| Path | Current role | Removal classification | Rationale |
|---|---|---|---|
| `toolchain/scripts/deploy/bin/aw-installer.js` | Primary Node bin. Owns help/version, `agents` package/local lifecycle, selected local agents unsupported-variant failures, `claude` package/local read-only/preflight lifecycle, `agents` GitHub-source JSON dry-run, TUI shell, environment filtering and Python fallback. | retain | This is the target owner for deploy runtime. Through Slice H, Node-owned behavior covers package/local agents `diagnose`, `check_paths_exist`, `verify`, clean and non-clean-conflict `install`, recognized same-backend managed-install `prune --all`, `update --backend agents` dry-run, TUI dry-run display, `update --backend agents --yes` composition, and selected local invalid variants such as prune without `--all` and update `--json --yes`. Slice I-claude-readonly covers package/local Claude diagnose human/JSON, `--claude-root`, check_paths_exist, verify and update dry-run human/JSON. Slice J-github-json covers explicit `agents` GitHub-source JSON dry-run with repo/ref/SHA validation, safe ZIP extraction, source contract validation, temp cleanup and source/target separation. GitHub-source human dry-run/apply, Claude mutating install/prune/update apply and other unmigrated deploy modes still fall back to Python/reference. |
...
|---|---|---|
| `toolchain/scripts/test/aw_installer_cli/test_cli_commands.py` | CLI help/version/default, `agents` and `claude` lifecycle, GitHub-source JSON Node-owned invalid-input sentinel, TUI non-interactive guard. | split-later; convert lifecycle expectations to Node-owned parity while keeping GitHub apply/Claude fallback tests until migrated. |
...
9. Slice I: Claude mutating local/package lifecycle. Candidate scope: clean-target `install --backend claude`, `prune --all --backend claude`, and `update --backend claude --yes`, preserving full Harness skill payload, `.claude/skills/<skill_id>` target naming, legacy `aw-<skill_id>` managed cleanup recognition, destructive recovery semantics, strict verify and failing-Python sentinel evidence. This must not promote Claude to the `agents` main path, change release channel, or remove Python runtime files.
10. Slice J: GitHub-source JSON dry-run migration. Status: verified for explicit `agents` `update --json --source github` planning, including repo/ref/SHA validation, safe zip extraction, source contract validation, temp cleanup, target/source separation and no auto-upgrade semantics. It does not cover GitHub-source human dry-run or `--yes` apply.
11. Slice K: GitHub-source apply/human dry-run migration. Candidate scope: Node-owned `--source github` human-readable dry-run and `update --backend agents --source github --yes`, preserving destructive reinstall semantics, post-apply verify and recovery hints on top of the already verified source archive trust boundary.
12. Slice L: runtime Python fallback and package payload retirement. Candidate scope: remove or reimplement `aw-harness-deploy`, remove `aw-installer` Python fallback branches, drop `harness_deploy.py` / `adapter_deploy.py` from package runtime `files`, and update closeout/release smoke to prove no-Python registry/package execution. This crosses package metadata, packlist and release boundary; it requires explicit approval and release-channel gates. `adapter_deploy.py` may remain in the repository as reference after it exits the package runtime payload.
...
- No GitHub-source human dry-run or apply migration before a dedicated follow-up slice.
