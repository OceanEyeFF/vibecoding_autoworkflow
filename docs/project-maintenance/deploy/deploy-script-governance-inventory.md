...
| Path | Current role | Removal classification | Rationale |
|---|---|---|---|
| `toolchain/scripts/deploy/bin/aw-installer.js` | Primary Node bin. Owns help/version, `agents diagnose` human/JSON, package-local `agents update` JSON and human-readable dry-run, package/local `agents check_paths_exist`, `verify`, `install`, `prune --all`, `update --yes` composition, selected local agents unsupported-variant failures, TUI shell, environment filtering and Python fallback. | retain | This is the target owner for deploy runtime. Through Slice H, Node-owned behavior covers package/local agents `diagnose`, `check_paths_exist`, `verify`, clean and non-clean-conflict `install`, recognized same-backend managed-install `prune --all`, `update --backend agents` dry-run, TUI dry-run display, `update --backend agents --yes` composition, and selected local invalid variants such as prune without `--all` and update `--json --yes`; GitHub-source, Claude and other unmigrated deploy modes still fall back to Python/reference. |
...
- a package-local no-Python sentinel smoke that shadows `py`, `python`, and `python3` with failing binaries and proves the currently claimed Node-owned `agents` package-local commands do not invoke Python. Through Slice H this covers `diagnose` human/JSON, `update --json`, human-readable `update`, TUI dry-run display, `check_paths_exist`, `verify`, `install` clean-target success and non-clean planned-path conflict blocking, `prune --all`, `update --yes`, and selected local invalid-variant failures; GitHub-source update, Claude backend and unmigrated variants remain Python/reference paths until separately implemented and approved.
