...
- When validating the current checkout, an explicit source checkout, or a maintainer-provided local `.tgz` built from this checkout, `aw-installer --help`, `--version`, `agents` package/local lifecycle commands, and `claude` package/local read-only/preflight commands are handled by Node directly. For `agents`, this includes diagnose human/JSON, update dry-run human/JSON, check_paths_exist, verify, install, prune --all, update --yes, and selected invalid variants. For `claude`, this includes diagnose human/JSON, `--claude-root`, check_paths_exist, verify, and update dry-run human/JSON; Claude install, prune --all and update --yes still use the Python reference/fallback path. Node-owned install writes only after source/target/path preflight passes; planned path conflicts fail before writes and do not invoke Python fallback. Node-owned prune only removes recognized current-backend managed install directories and retains foreign, unrecognized, invalid-marker and user content. Node-owned `update --yes` for agents composes `prune --all -> check_paths_exist -> install -> verify` with blocking preflight, strict post-apply verify and recovery hint semantics. Published registry artifacts get these checkout-only behaviors only after a future approved release. GitHub-source update, Claude mutating commands, and unmigrated unsupported variants still use the Python deploy wrapper/reference path. The Node-owned update dry-run preserves the existing JSON fields, including `backend`, `source_kind`, `source_ref`, `source_root`, `target_root`, `operation_sequence`, `managed_installs_to_delete`, `planned_target_paths`, `issues`, and `blocking_issues`, and the human-readable output remains dry-run only. The current wrapper tries `py -3`, `python`, then `python3` on Windows, and only `python3` on Linux/macOS for fallback paths. It intentionally ignores `PYTHON` and `PYTHON3` environment overrides.
