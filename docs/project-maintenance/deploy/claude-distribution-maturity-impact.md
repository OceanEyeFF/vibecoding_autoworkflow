...
> 目的：把 “Claude 分发已经通过 Python adapter 落地 full Harness skill payload” 这一当前事实，与后续 Node-owned CLI / TUI / registry / release 成熟度工作分开，避免继续使用把 Claude 写成未落地或未来化的过期表述。
...
- current_fact: `adapter_deploy.py --backend claude` 已经承接受控的完整 Harness skill payload lane。
...
- maturity_limit: 该成熟度不自动等价于 Node-owned `aw-installer` 主路径、TUI backend 选择、公网 registry smoke、stable/latest release 或 Claude 替代 `agents` 主线已经成熟。
...
- Node-owned `aw-installer` CLI 对 `claude` backend 的同等主路径承接。
- TUI 将 Claude backend 作为一等选择路径，而不是围绕 `agents` 主路径和 Python fallback 保持兼容。
...
- Claude full payload 已由 Python adapter lane 落地。
- Claude 仍是 `agents` mainline 旁路的 compatibility lane。
- Node-owned CLI / TUI / release maturity 仍需单独证据和 worktrack。
...
Python adapter 目前是 Claude maturity 的实现 owner。Node-owned `aw-installer` 与 TUI 仍应按独立 feature/config worktrack 处理：

- CLI: 明确哪些 `claude` command 走 Node-owned path，哪些仍 fallback。
...
承认 Claude full payload 已在 Python adapter lane 落地，并把旧的未实现/未来化口径改成 “adapter-backed compatibility lane 已成熟”。本页覆盖当前阶段。

### Phase B: 证据补强

在不发布、不改 dist-tag、不推 remote 的前提下，为 Claude lane 增补临时 target repo、package `.tgz` 或 registry candidate smoke 证据。

### Phase C: Node / TUI 成熟化

如果要让 Claude 进入 Node-owned `aw-installer` 主路径或 TUI 一等路径，开独立 feature/config worktrack，并明确 fallback、测试和 operator-facing 文案。
...
- 修改代码行为、Node-owned CLI、TUI 或 adapter semantics：停止，开 feature/config worktrack。
