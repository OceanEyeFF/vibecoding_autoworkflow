...
- 根目录 `package.json` 是 self-contained `aw-installer` package envelope；`aw-installer` 是已批准的 unscoped public package identity。`aw-installer@0.4.0-rc.1` 已发布到 npm `latest`，`aw-installer@0.4.0-rc.2` 已发布到 npm `next`；当前 checkout 的 next RC candidate 是 `0.4.1-rc.1`。
- `0.4.0-rc.1` 是首个已发布 `0.4.x` RC checkpoint；`P0-019` 已跨过该版本的真实 publish 审批边界。`0.4.1-rc.1` 是后续 `next` RC candidate，绑定 `v0.4.1-rc.1` 与 root package approval lock。后续 publish 仍必须同时满足本文的环境、tag、dist-tag、CI、package metadata approval lock、GitHub master source readiness 与 registry 准入条件。
...
- 当前 registry 事实是 `next: 0.4.0-rc.2` 与 `latest: 0.4.0-rc.1`。面向已发布 RC 试用必须显式使用 `aw-installer@next`；裸 `npx aw-installer` 仍按 npm `latest` 解析到 rc1。`0.4.1-rc.1` 在 develop-aw 完成后，还需要按仓库规则通过 develop PR 进入 GitHub `master`，再执行真实 npm `next` publish。
...
Real publish requires a separate approval boundary, an explicit tracked metadata-lock change, and explicit release metadata. The prepared `0.4.1-rc.1` metadata lock is:
...
The `0.4.1-rc.1` command shape, after GitHub `master` source readiness is proven, is:
...
The `0.4.1-rc.1` publish will move npm `next` to `0.4.1-rc.1`; until that publish happens, npm `next` remains `0.4.0-rc.2` and npm `latest` remains `0.4.0-rc.1`. Treat `latest` as the older rc1 selector, not as stable-release approval.
