...
- `install --backend agents` 只消费当前 live `manifest + payload`，并把 canonical skill copy、payload descriptor 与 runtime-generated `aw.marker` 写入当前 backend 的 resolved target root
