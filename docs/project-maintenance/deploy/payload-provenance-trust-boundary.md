...
`aw-installer` Node bin 当前直接承接 `--help`、`--version` 与 `diagnose --backend agents` human/JSON 的只读路径；TUI 的 agents diagnose 展示也复用该 Node-owned JSON 路径。`update --backend agents --json` 与 human-readable `update --backend agents` 在 package/local source dry-run 场景下也由 Node-owned 路径生成同一 plan，TUI 的 dry-run 展示复用该 Node plan。`check_paths_exist --backend agents` 在 package/local source 场景下由 Node-owned 路径执行只读冲突扫描，并保留 Python reference 的 source validation、target root readiness、duplicate target dir、conflict reporting、exit、stdout 和 stderr 语义。`verify --backend agents` 在 package/local source 场景下由 Node-owned 路径执行 strict verification，TUI 的 agents verify action 也复用该路径，并保留 Python reference 的 target issue classification、exit、stdout 和 stderr 语义。`install --backend agents` 在 package/local source 场景下由 Node-owned 路径执行 clean-target 写入和 non-clean target path-conflict 阻断；它仍必须先完成 source validation、target readiness 与 path conflict preflight，再创建 target root、写入 payload、写入 runtime marker，并保留 Python reference 的 exit、stdout、stderr 与 file/dir mode 语义；冲突路径必须在写入前失败且不调用 Python fallback。`prune --all --backend agents` 在 package/local source 场景下由 Node-owned 路径执行删除；它只删除带可识别 `aw.marker` 且 marker backend 为 `agents` 的 direct child 目录，保留 foreign、unrecognized、invalid-marker、file 和 symlink content，并保留 Python reference 的 missing-root no-op、target readiness、race guard、exit、stdout 和 stderr 语义。`update --backend agents --yes` 在 package/local source 场景下由 Node-owned 路径执行 destructive reinstall composition；它先输出同一 plan，再按 `prune --all -> check_paths_exist -> install -> verify` 串行执行，保留 blocking preflight、failure short-circuit、post-apply strict verify、recovery hint、exit、stdout 和 stderr 语义。该 dry-run JSON 必须继续暴露 `backend`、`source_kind`、`source_ref`、`source_root`、`target_root`、`operation_sequence`、`managed_installs_to_delete`、`planned_target_paths`、`issues` 与 `blocking_issues` 等字段。`prune --backend agents` 缺少 `--all` 和 `update --backend agents --json --yes` 等本地 agents 无效组合由 Node 直接失败。其他 deploy modes、不受支持的 diagnose / update / check_paths_exist / verify / install / prune 变体、GitHub source update 与 Claude backend 仍调用同包内的 Python wrapper/reference path；fallback 环境只透传 deploy 所需变量，不全量继承 shell secrets。实际 source / target 解析必须保持与 `adapter_deploy.py` 的合同一致：
...
- `diagnose`、`verify` 仍是只读命令；`agents` package/local diagnose human/JSON 路径支持 `--agents-root` 且不调用 Python fallback。
- `update --backend agents --json` 与 human-readable `update --backend agents` 在 package/local source dry-run 场景下由 Node-owned 路径输出 plan，并保留既有 JSON 字段和 Python-compatible human-readable 输出形状。
- `check_paths_exist --backend agents` 在 package/local source 场景下由 Node-owned 路径输出 Python-compatible preflight result；成功和冲突路径必须不调用 Python fallback。
- `verify --backend agents` 在 package/local source 场景下由 Node-owned 路径输出 Python-compatible strict verification result；成功、drift 和 target issue paths 必须不调用 Python fallback。
- `install --backend agents` 在 package/local source 场景下由 Node-owned 路径写入 payload或阻断冲突；缺失或空 target root 成功路径、non-clean planned-path conflict 失败路径都必须不调用 Python fallback。
- `prune --all --backend agents` 在 package/local source 场景下由 Node-owned 路径删除 current-backend managed installs；missing target root、same-backend marker deletion、foreign/unrecognized/invalid retention 和 target root readiness failures 必须不调用 Python fallback。
- `update --backend agents --yes` 在 package/local source 场景下由 Node-owned 路径执行 destructive reinstall；必须证明 no-Python sentinel、blocking preflight、strict post-apply verify、recovery hint 和 Python reference output shape。
