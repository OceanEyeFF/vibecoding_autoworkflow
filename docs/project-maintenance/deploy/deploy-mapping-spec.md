...
- 映射链路本身
- payload descriptor 的最小字段
- deploy 命令依赖这些字段做什么

## 本页不管理什么

- operator 执行步骤：见 [Deploy Runbook](./deploy-runbook.md)
- wrapper 入口语义：见 [Distribution Entrypoint Contract](./distribution-entrypoint-contract.md)
- source root / target root / remote source trust boundary：见 [aw-installer Payload Provenance And Update Trust Boundary](./payload-provenance-trust-boundary.md)
...
`canonical_dir`、`target_dir`、`target_entry_name` 和 `required_payload_files` 都必须是安全的相对路径，不能跳出各自根目录。
...
- operator 执行步骤
- wrapper 入口语义
- source root / target root / remote source trust boundary
