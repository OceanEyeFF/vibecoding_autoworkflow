...
`toolchain/scripts/deploy/package.json`、`bin/aw-installer.js` 和 `bin/aw-harness-deploy.js` 仍保留为本地 npm-style scaffold。`aw-installer` 是主 bin，`aw-harness-deploy` 是兼容别名。`aw-installer --help` / `--version` 由 Node wrapper 直接处理；`aw-installer diagnose --backend agents` human/JSON 当前也有 Node-owned 只读路径，TUI 的 diagnose 展示也必须复用该路径；`aw-installer update --backend agents --json` 和 human-readable `aw-installer update --backend agents` 在 package/local source dry-run 场景下由 Node-owned 路径生成同一 plan；`aw-installer check_paths_exist --backend agents` 在 package/local source 场景下也由 Node-owned 只读路径执行 target path conflict scan；`aw-installer verify --backend agents` 在 package/local source 场景下由 Node-owned 只读路径执行 strict target verification，TUI 的 agents verify action 也必须复用该路径；`aw-installer install --backend agents` 在 package/local source 场景下由 Node-owned 路径执行 clean-target 写入和 non-clean target path-conflict 阻断，并保留 Python reference 的 exit/stdout/stderr 语义；`aw-installer prune --all --backend agents` 在 package/local source 场景下由 Node-owned 路径删除 current-backend managed installs，并保留 Python reference 的 missing-root no-op、foreign/unrecognized retention、race guard、exit/stdout/stderr 语义；`aw-installer update --backend agents --yes` 在 package/local source 场景下由 Node-owned 路径执行同一 destructive reinstall composition，先打印 plan，再按 `prune --all -> check_paths_exist -> install -> verify` 串行执行，并保留 Python reference 的 blocking preflight、failure short-circuit、post-apply strict verify、recovery hint、exit/stdout/stderr 语义。该 update JSON 必须保留 `backend`、`source_kind`、`source_ref`、`source_root`、`target_root`、`operation_sequence`、`managed_installs_to_delete`、`planned_target_paths`、`issues` 与 `blocking_issues` 等字段，并比较完整 issue 对象而不是只比较 code。`prune --backend agents` 缺少 `--all` 和 `update --backend agents --json --yes` 等本地 agents 无效组合由 Node 直接失败。其他 deploy modes、不受支持的 diagnose / update / check_paths_exist / verify / install / prune 变体、GitHub source update 与 Claude backend 仍调用同一个 Python wrapper/reference path。`0.4.3-rc.2` 在 Windows 上按 `py -3`、`python`、`python3` 尝试 Python launcher，在 Linux/macOS 上只尝试 `python3`；wrapper 不接受 `PYTHON`/`PYTHON3` 环境变量覆盖。Node wrapper 调用 Python fallback 时只透传 deploy 必需环境、路径/语言环境、代理与证书变量，不全量转发 shell 环境中的 token 或 API key。
...
当前 Node-owned 覆盖包括 `agents` package/local source 的 `diagnose` human/JSON、`check_paths_exist`、`verify`、clean-target `install`、non-clean target install 冲突阻断、`prune --all`、默认 human-readable `update` dry-run、`update --yes` composition，以及 selected local invalid-variant failures。`diagnose` 必须保持只读，发现 issue 仍可 0 退出，并支持 `--agents-root` 覆盖。`check_paths_exist` 必须继续复用 source validation、duplicate `target_dir`、target root readiness、legacy path conflict 和 planned path conflict 语义；缺失 target root 是允许的只读 preflight 状态，不得创建目录或写入 payload。`verify` 必须继续复用 strict verification 语义，覆盖 missing target root、wrong target root type、missing target entry、unrecognized target directory、required payload、target entry type、fingerprint drift、unexpected managed directory 等 issue，并以 issue 存在与否决定退出码。`install` 必须先完成 source validation、target readiness 与 path conflict preflight，再创建 target root、写入 payload、写入 marker 并保持 file/dir mode；non-clean target 的 planned path conflict 必须在写入前失败。`prune --all` 的 Node-owned 边界只覆盖 `agents` backend current-backend managed install 删除；它不得删除 foreign backend、missing/invalid marker、file、symlink 或 user content，并必须在 marker read 后复查目录 identity。默认 `update` 的 Node-owned 边界只覆盖 package/local `agents` source，且只打印 plan、不写入；`update --yes` 必须先打印 plan，blocking preflight 时不得 apply，apply 失败时必须打印 recovery hint，成功时必须 post-apply strict verify。`prune --backend agents` 缺少 `--all` 和 `update --backend agents --json --yes` 必须直接失败且不调用 Python。
