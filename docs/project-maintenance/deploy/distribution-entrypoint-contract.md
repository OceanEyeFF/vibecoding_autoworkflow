...
`toolchain/scripts/deploy/package.json`、`bin/aw-installer.js` 和 `bin/aw-harness-deploy.js` 仍保留为本地 npm-style scaffold。`aw-installer` 是主 bin，`aw-harness-deploy` 是兼容别名。`aw-installer --help` / `--version` 由 Node wrapper 直接处理；package/local `agents` lifecycle 当前由 Node-owned 路径承接，覆盖 diagnose human/JSON、update dry-run human/JSON、check_paths_exist、verify、clean-target install、non-clean target install 冲突阻断、prune --all、update --yes composition，以及 selected invalid-variant failures。package/local `claude` read-only/preflight 当前也由 Node-owned 路径承接，覆盖 diagnose human/JSON、`--claude-root`、check_paths_exist、verify 和 update dry-run human/JSON；Claude install、prune --all 与 update --yes 仍走 Python reference/fallback。显式 `agents` GitHub-source JSON dry-run（`update --backend agents --json --source github ...`）当前由 Node-owned 路径承接，覆盖 repo/ref/SHA validation、codeload archive URL 构造、safe ZIP extraction、source contract validation、temp cleanup 与 source/target separation；GitHub-source human dry-run 与 `--yes` apply 仍走 Python reference/fallback。TUI 的 agents diagnose、verify 和 dry-run 展示必须复用对应 Node-owned 路径。Node-owned update JSON 必须保留 `backend`、`source_kind`、`source_ref`、`source_root`、`target_root`、`operation_sequence`、`managed_installs_to_delete`、`planned_target_paths`、`issues` 与 `blocking_issues` 等字段，并比较完整 issue 对象而不是只比较 code。其他 deploy modes、不受支持的 diagnose / update / check_paths_exist / verify / install / prune 变体、GitHub-source apply/human dry-run 与 Claude mutating lifecycle 仍调用同一个 Python wrapper/reference path。`0.4.3-rc.2` 在 Windows 上按 `py -3`、`python`、`python3` 尝试 Python launcher，在 Linux/macOS 上只尝试 `python3`；wrapper 不接受 `PYTHON`/`PYTHON3` 环境变量覆盖。Node wrapper 调用 Python fallback 时只透传 deploy 必需环境、路径/语言环境、代理与证书变量，不全量转发 shell 环境中的 token 或 API key。
...
当前 Node-owned 覆盖包括 `agents` package/local source 的 `diagnose` human/JSON、`check_paths_exist`、`verify`、clean-target `install`、non-clean target install 冲突阻断、`prune --all`、默认 human-readable `update` dry-run、`update --yes` composition，以及 selected local invalid-variant failures；同时包括 `claude` package/local source 的 `diagnose` human/JSON、`check_paths_exist`、`verify` 和默认 human-readable / JSON `update` dry-run。`diagnose` 必须保持只读，发现 issue 仍可 0 退出，并支持当前 backend 的 root override（`--agents-root` 或 `--claude-root`）。`check_paths_exist` 必须继续复用 source validation、duplicate `target_dir`、target root readiness、legacy path conflict 和 planned path conflict 语义；缺失 target root 是允许的只读 preflight 状态，不得创建目录或写入 payload。`verify` 必须继续复用 strict verification 语义，覆盖 missing target root、wrong target root type、missing target entry、unrecognized target directory、required payload、target entry type、fingerprint drift、unexpected managed directory 等 issue，并以 issue 存在与否决定退出码；Claude verify 还必须保留 `claude_frontmatter` target transform parity，避免对 Python/reference installed payload 产生 false drift。`install` 必须先完成 source validation、target readiness 与 path conflict preflight，再创建 target root、写入 payload、写入 marker 并保持 file/dir mode；non-clean target 的 planned path conflict 必须在写入前失败。`prune --all` 的 Node-owned 边界当前只覆盖 `agents` backend current-backend managed install 删除；它不得删除 foreign backend、missing/invalid marker、file、symlink 或 user content，并必须在 marker read 后复查目录 identity。默认 `update` 的 Node-owned 边界覆盖 package/local `agents` 与 `claude` dry-run，且只打印 plan、不写入；`update --yes` 当前只对 agents Node-owned，必须先打印 plan，blocking preflight 时不得 apply，apply 失败时必须打印 recovery hint，成功时必须 post-apply strict verify。`prune --backend agents` 缺少 `--all` 和 `update --backend agents --json --yes` 必须直接失败且不调用 Python。Claude install、prune --all 和 update --yes 仍走 Python reference/fallback。
