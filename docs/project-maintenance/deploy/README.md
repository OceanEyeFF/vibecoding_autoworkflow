...
- 当前 registry 事实由 [aw-installer Release Channel Contract](./release-channel-contract.md) 承接：`next` 当前指向 `0.4.1-rc.2`，`latest` 仍指向 `0.4.0-rc.1`；`0.4.1-rc.2` 已是 published RC registry artifact，不得对同一 immutable npm version 重复 publish。
