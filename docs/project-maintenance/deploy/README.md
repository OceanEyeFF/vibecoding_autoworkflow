...
- 当前 registry 事实由 [aw-installer Release Channel Contract](./release-channel-contract.md) 承接：`next` 当前指向 `0.4.1-rc.2`，`latest` 仍指向 `0.4.0-rc.1`；已发布的 `0.4.1-rc.2` artifact 绑定 `gitHead=7f7536a`，不得对同一 immutable npm version 重复 publish。当前本地后续变化若要进入 registry，必须先升新版本。
