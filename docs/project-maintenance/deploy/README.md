...
- 当前 registry 事实由 [aw-installer Release Channel Contract](./release-channel-contract.md) 承接：`next` 当前指向 `0.4.1-rc.3`，`latest` 仍指向 `0.4.0-rc.1`；已发布的 `0.4.1-rc.3` artifact 绑定 `gitHead=dc67d2e2bb54c02aa254f5c058b4eeae1bd8afc2`，不得对同一 immutable npm version 重复 publish。当前本地 release-prep candidate 是 `0.4.2-rc.0`。
