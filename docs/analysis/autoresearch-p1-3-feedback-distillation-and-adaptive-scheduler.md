...
- `feedback-distill.json` 负责描述“这一轮的 distilled 结果”，并保留 rich round artifact
- `feedback-ledger.jsonl` 负责把多轮 feedback 累积成可搜索、可排序、可筛选的 family 级历史，但只保留 compact aggregate summary

## 五、Distilled Feedback 的对象模型

### 1. 最小字段

当前代码落地后，round distill 与 run ledger 已明确拆成两套 contract。

round `feedback-distill.json` 最小字段包括：
...
run `feedback-ledger.jsonl` 的 compact entry 最小字段则收紧为：

- 身份：`run_id / round / mutation_key / mutation_id / attempt`
- 结果：`decision / *_delta / signal_strength / regression_flags`
- family 级摘要：`dimension_feedback_summary / aggregate_prompt_guidance`
- 溯源：`scoreboard_ref / decision_ref / worker_contract_ref / distilled_at`
...
当前实现里，下面字段也已经固定为 deterministic script output，而不是模型自由总结：
...
- selector 和 `recent_feedback_excerpt` 只消费 ledger compact summary
- 下一轮 worker contract 不读取 repo 明细，只读取 latest `aggregate_prompt_guidance`
