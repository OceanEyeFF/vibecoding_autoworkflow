...
`worker-contract.json` 必须只依赖下面四类输入：

- `contract.json`
- `mutation-registry.json`
- round `mutation.json`
- `runtime.json` / `round.json` / `worktree.json`
...
`selection_reason` 只需要是固定枚举或短字符串，例如：
...
5. 再按 registry 原始顺序或 `mutation_key` 排序做 tie-break
...
### 2. 子阶段 B：实现 worker contract schema + generator

进入 `toolchain/`：

- 新增 `autoresearch-worker-contract.schema.json`
- 新增 `autoresearch_worker_contract.py`
- 在 `prepare-round` 结束时写出 `worker-contract.json`

验收信号：

- agent 可以只消费 worker contract 完成本轮
- `run-round` 可以校验 worker contract 锚点仍然一致

### 3. 子阶段 C：实现 minimal selector

进入 `toolchain/scripts/research/`：

- 新增 `autoresearch_selector.py`
...
- selector 输出仍然只是 `mutation_key`，没有长成调度平台
