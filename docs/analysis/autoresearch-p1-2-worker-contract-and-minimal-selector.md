...
- 当前代码里 `build_worker_contract_payload()` 直接消费的是 `contract.json + round mutation.json + round.json + baseline scoreboard`；registry / runtime / worktree 约束则通过 `prepare-round` 的 materialization、round authority snapshot 和 `run-round` 校验链维持
