...
- `OpenCode`：MVP 走 `opencode run`，透传 `model / --dir`，并把统一 output format 归一化到 OpenCode 的 `default/json`；`extract_final_message()` 从 JSON event stdout 提取最后一个 assistant message，schema judge 仍不支持
