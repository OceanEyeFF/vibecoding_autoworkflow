...
> 目的：定义 `Task Contract` 作为 `Task Interface` 对象的职责和边界，并说明它如何被固定为可直接调用的 skill 载体，而不把它并入 `Memory Side` 或 Harness 本体。
...
- 它能作为执行前唯一正式基线
- 它能被不同后端用同一套结构消费
- 它不会被误写成 `Memory Side` 组件或 Harness runtime 方案
