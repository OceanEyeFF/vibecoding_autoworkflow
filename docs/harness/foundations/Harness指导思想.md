...
- `docs/project-maintenance/deploy/` 负责 repo-local deploy / verify 的 operator-facing 边界

更具体的 repo-local adapter、runtime state file、deploy 绑定，应继续落在 `product/memory-side/`、`product/task-interface/` 与 `docs/project-maintenance/`，而不是继续堆进这份指导思想。
