...
- 当前处于 `RepoScope` 还是 `WorktrackScope`
- 当前状态允许哪个 `Function` 算子
- 需要消费哪些 `Artifact`
...
单个 skill 的 bounded round 只限制本轮局部动作，不自动等价于整个 Harness 停机。只要没有命中正式 stop condition，supervisor 应继续推进到下一个合法状态转移。
...
- 当前 host runtime 没有合法 execution carrier / dispatch shell
- 下一动作会越过已批准的 `Task Contract`、`Worktrack Contract` 或 repo baseline
- 同一个交接边界在连续无变化轮次中再次被确认

补充约束：

- “skill 已返回结构化结果”不是 stop condition。
...
`Dispatch` 必须在专用 skill、通用 `SubAgent` 和 current-carrier fallback 之间保持同一份最小合同。

执行载体开关：

- 默认值是 `auto`
- `subagent_dispatch_mode` 支持 `auto | delegated | current-carrier`
- `subagent_dispatch_mode_override_scope` 默认是 `worktrack-contract-primary`
- `worktrack-contract-primary` 表示当前 `Worktrack Contract` 的 `runtime_dispatch_mode` 优先
- 只有显式 `global-override` 时，`.aw/control-state.md` 的 `subagent_dispatch_mode` 才压过 worktrack contract
- 若 worktrack 未声明 `runtime_dispatch_mode`，再使用 control-state default
...
没有匹配专用 skill 时，`dispatch-skills` 可以生成一次性任务指令并绑定通用执行载体，但不得把这类指令写成新的 canonical skill。

## 八、Verify 与 Gate

`Verify` 收集证据，`Judge` 做放行裁决。二者不能混成一层。
...
- 不把 `Task Contract`、`Route Card` 或 `Writeback Card` 改写成 Harness 本体。
- 不把 `Skill` 当作上位 ontology；`Skill` 只是 `Function` 的实践实现。
...
- 不把未验证结论写回长期 truth layer。

## 十一、与当前仓库主线的关系

- `docs/harness/` 承接 Harness doctrine、protocol、artifact、scope、catalog 和 workflow family。
- `product/harness/` 承接 executable source，不承接 ontology 正文。
- `docs/harness/adjacent-systems/task-interface/task-contract.md` 是任务执行基线，不是 Harness 本体。
- `docs/harness/adjacent-systems/memory-side/` 承接长期记忆、路由和写回边界，是 adjacent system。
...
- `subagent_dispatch_mode` 与 `runtime_dispatch_mode` 是可开关合同，不是硬编码行为。
- `Evidence` 与 `Gate` 分开。
- Gate fail 有明确 recovery route。
- Closeout 以 repo refresh 结束，而不是以 PR 创建结束。
