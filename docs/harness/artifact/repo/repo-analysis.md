...
Repo Analysis 是 RepoScope 的决策支撑 artifact，把仓库事实压缩成可复核的优先级判断。它回答当前仓库事实、推断/未知项、主要矛盾、最高优先级、路由投影、可写回长期 truth 的结论；不回答新目标、worktrack 任务队列、实现切片编码、或未验证结论是否应进入长期文档。

## Position

Repo Analysis 位于 Goal/Charter 与 Snapshot/Status 下游：Goal 定义长期参考信号和 Engineering Node Map，Snapshot 定义当前慢变量状态，Repo Analysis 基于两者做阶段性矛盾分析和优先级判断；repo-whats-next-skill 可消费它但不能当成目标真相替代品。

## Required Inputs

一份合格的 Repo Analysis 至少引用 Goal/Charter、Snapshot/Status、Control State、本轮必要的入口文档或模块证据，以及影响判断的最近 worktrack contract/plan/evidence 最小边界摘要。禁止把 deploy target、本地挂载结果或未验证报告当作唯一事实来源。
...
事实、推断、未知项必须分开写；当前主要矛盾和最高优先级只能各有一个；证据不足以选择主要矛盾时 continuation_ready 必须为 false 并列出缺失信息；不要把 task list 当成 repo 分析；不要把分析报告写成新的 Goal Charter（目标变更只能走 repo-change-goal-skill）。
...
4. 进入 WorktrackScope.Init 时 worktrack contract 只能引用分析结论，不能复制全文
