...
> `.aw/repo/worktrack-backlog.md` 是 RepoScope 运行时 artifact，记录所有 worktrack 的完成状态。它不是部署模板——由 repo-refresh-skill 在 worktrack closeout 后写入（upsert-by-worktrack_id），harness-skill 作为 supervisor/binder 负责调度 repo-refresh，由 milestone-status-skill 在 Milestone Observe 时消费。
