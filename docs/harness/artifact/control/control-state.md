...
这些字段属于 control policy，不回答 repo 目标或替代 Worktrack Contract。subagent_dispatch_mode 语义必须与 worktrack 级 runtime_dispatch_mode 保持一致：auto 优先委派否则显式 runtime fallback；delegated 必须委派否则返回 gap/block；current-carrier 显式关闭委派。
