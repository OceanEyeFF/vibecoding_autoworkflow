...
- 禁止未授权降级：不得在未明确说明和未获确认的情况下，自行 fallback 到“简单但不完整”的方案。
...
并且在 Step 1 结束前，必须新增“风险预分析”：
- Blocking Risks：会直接阻塞任务推进的风险（依赖、环境、接口、权限、数据条件等）
- Rework Risks：不提前处理会导致频繁返工的风险（需求歧义、边界模糊、测试口径不一致、隐藏耦合等）
- 每个风险必须写明处理策略：立即处理 / 延后处理（含理由）/ 请求人工决策
...
4. Test Gate：
   - 可补白盒测试则必须补。
   - 在 strict 模式下，若可执行覆盖率统计，必须先让使用者选择白盒覆盖率目标：`90%` / `100%` / `AI决定`。
   - 若用户选择 `AI决定`，需在报告中给出推荐覆盖率和理由。
   - 不能补则必须给出明确原因与风险。
5. Smoke Gate：
   - 可执行最小烟测则必须执行。
   - 不能执行则必须说明阻塞点与替代验证证据。
