...
# 记录开始日志
echo "{\"ts\":\"$(date -u +%Y-%m-%dT%H:%M:%SZ)\",\"tool\":\"claude-code\",\"session\":\"$SESSION_ID\",\"kind\":\"agent_start\",\"agent\":\"code-analyzer\",\"task\":\"<任务描述>\"}" >> .autoworkflow/logs/claude-code/feedback.jsonl
```
...
echo "{\"ts\":\"$(date -u +%Y-%m-%dT%H:%M:%SZ)\",\"tool\":\"claude-code\",\"session\":\"$SESSION_ID\",\"kind\":\"agent_end\",\"agent\":\"code-analyzer\",\"status\":\"success\",\"duration_ms\":<耗时毫秒>,\"summary\":\"<执行摘要>\"}" >> .autoworkflow/logs/claude-code/feedback.jsonl
```
...
echo "{\"ts\":\"$(date -u +%Y-%m-%dT%H:%M:%SZ)\",\"tool\":\"claude-code\",\"session\":\"$SESSION_ID\",\"kind\":\"error\",\"agent\":\"code-analyzer\",\"error\":\"<错误描述>\"}" >> .autoworkflow/logs/claude-code/feedback.jsonl
```
...
**示例**：
- ✅ 正确：`"task":"分析 src 目录的代码质量"`
- ❌ 错误：`"task":"分析 "src/" 目录的代码质量"` （包含双引号）
- ❌ 错误：`"task":"分析\nsrc/"` （包含换行符）
...
jq -n \
  ...
  --arg agent "code-analyzer" \
  --arg task "分析 \"src/\" 目录的代码质量（包含特殊字符）" \
