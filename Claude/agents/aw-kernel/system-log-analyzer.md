...
# 记录开始日志
echo "{\"ts\":\"$(date -u +%Y-%m-%dT%H:%M:%SZ)\",\"tool\":\"claude-code\",\"session\":\"$SESSION_ID\",\"kind\":\"agent_start\",\"agent\":\"system-log-analyzer\",\"task\":\"<任务描述>\"}" >> .autoworkflow/logs/claude-code/feedback.jsonl
```
...
echo "{\"ts\":\"$(date -u +%Y-%m-%dT%H:%M:%SZ)\",\"tool\":\"claude-code\",\"session\":\"$SESSION_ID\",\"kind\":\"agent_end\",\"agent\":\"system-log-analyzer\",\"status\":\"success\",\"duration_ms\":<耗时毫秒>,\"summary\":\"<执行摘要>\"}" >> .autoworkflow/logs/claude-code/feedback.jsonl
```
...
echo "{\"ts\":\"$(date -u +%Y-%m-%dT%H:%M:%SZ)\",\"tool\":\"claude-code\",\"session\":\"$SESSION_ID\",\"kind\":\"error\",\"agent\":\"system-log-analyzer\",\"error\":\"<错误描述>\"}" >> .autoworkflow/logs/claude-code/feedback.jsonl
```
...
jq -n \
  ...
  --arg agent "system-log-analyzer" \
  --arg task "分析 \"error.log\" 文件（包含特殊字符）" \
