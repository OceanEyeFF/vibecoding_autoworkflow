...
- 一套围绕 `autoresearch/src/` 组织起来的 contract-driven research loop
...
`autoresearch/src/` 是当前 `autoresearch` 实现面的 source of truth；
`autoresearch/docs/knowledge/` 只负责模块知识入口，不替代实现文档，也不扩写实现细节。
...
4. 只有任务明确落到实现或 CLI 内部接线时，再读 [autoresearch/src/README.md](../../../autoresearch/src/README.md)。

### 2. 修改实现时

1. 先读本页。
2. 再读 [autoresearch/src/README.md](../../../autoresearch/src/README.md)。
3. 再进入目标脚本、测试和 fixture。
4. 只有需要阶段语义或实现边界时，再进入对应 `autoresearch/src/` 源码与测试。
...
- [AGENTS.md](../../../AGENTS.md)
- [Autoresearch 最小闭环运行说明](../runbooks/autoresearch-minimal-loop.md)
- [Research Scripts](../../../autoresearch/src/README.md)
