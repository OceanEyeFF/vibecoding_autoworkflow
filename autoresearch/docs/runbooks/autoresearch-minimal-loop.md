...
- 独立的 P2 deterministic smoke 已固定在 `autoresearch/src/test_autoresearch_p2_smoke.py`。注意当前 smoke 是 orchestration smoke：lane 执行使用 mock runner，只覆盖 CLI 编排与 decision/replay 产物面
...
- `autoresearch/src/exrepo_runtime.py` 已提供稳定的 `/tmp` exrepo 根目录 helper 与 suite 物化 helper
...
runs:
  - repo: typer
    task: context-routing
    prompt_file: /abs/path/to/repo/autoresearch/src/tasks/context-routing-skill-prompt.md
```
...
```bash
python3 autoresearch/src/run_autoresearch_loop.py \
  --contract /abs/path/to/contract.json \
...
- [autoresearch/src/README.md](../../../autoresearch/src/README.md)
