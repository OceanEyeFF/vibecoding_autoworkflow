...
- `autoresearch/src/manage_tmp_exrepos.py`

它当前只负责：

- 读取 `autoresearch/src/exrepo.txt` 或显式 `--repo-list`
...
- `--repo-list` 默认指向 `autoresearch/src/exrepo.txt`
...
- `autoresearch/src/exrepo_runtime.py`
...
- `autoresearch/src/test_manage_tmp_exrepos.py`
  - repo list 解析
...
- `python3 autoresearch/src/manage_tmp_exrepos.py --help`
  - 顶层 shared options
  - `init / reset / prepare` 子命令存在
- `python3 autoresearch/src/manage_tmp_exrepos.py <subcommand> --help`
  - `--repo` / `--suite` 只存在于子命令
- `autoresearch/src/common.py`
...
- [路径与文档治理检查运行说明](../../../docs/project-maintenance/governance/path-governance-checks.md)
