...
`autoresearch/src/run_skill_suite.py` 在传入 `--save-dir` 时，会创建时间戳命名的 run 目录，并为每个 task phase 写出一组稳定产物：
...
- `*.attempts.json`：当 phase 发生 retry 时，记录每次 attempt 的失败原因、stdout/stderr 摘要和最终采用的 attempt。
...
`OpenCode` 现在也纳入这套 observability contract：

- `raw_stdout` / `raw_stderr` 仍保留 CLI 原始输出
- `final_message` 由 backend adapter 从 JSON event stdout 中提取最后一个 assistant message
- 若事件提取失败，仍回退 `stdout.strip()`
...
如果当前 phase 发生 retry，还会稳定记录：

- `attempt_count`
- `final_attempt`
- `failure_reason`
- `attempts[]`：每次 attempt 的 `failure_reason / parse_error / stdout_excerpt / stderr_excerpt / timing`
...
- `results[].attempt_count` / `results[].final_attempt`：terminal result 最终采用了第几次 attempt
- `results[].failure_reason`：terminal result 的失败分类；成功时为 `null`
- `results[].attempts`：每次 attempt 的精简 history；不会把 attempt 展平成多条 result
