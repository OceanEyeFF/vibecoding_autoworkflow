...
```
toolchain/docs/aw-kernel/
├── README.md              # 本文件 - 目录说明
├── GUIDE.md              # Agent 全局配置与使用指南
├── CHANGELOG.md           # 变更日志
├── STANDARDS.md           # 失败处理规范
├── LOGGING.md             # 日志系统说明
...
| 文档 | 用途 | 何时读 |
|------|------|--------|
| **GUIDE.md** | Agent 全局配置与使用指南 | 首次使用 aw-kernel Agent / 不确定 Agent 职责时 |
...
- **Agent 定义**: `toolchain/agents/aw-kernel/` - Agent 的实际定义文件（12个）
- **资源模板**: `toolchain/assets/aw-kernel/templates/` - Agent 使用的模板资源
- **Skills**: `toolchain/skills/aw-kernel/` - 工作流编排（如 autodev）
...
1. **读这些文档**:
   - [GUIDE.md](GUIDE.md) - 了解 Agent 全局配置和职责
   - 项目根目录的 [GUIDE.md](../../../GUIDE.md) Part 2 - 了解路由表

2. **选择 Agent**:
   - 查看项目根 GUIDE.md Part 2 的按任务类型路由表
   - 选择合适的 Agent（在 `toolchain/agents/aw-kernel/` 目录）
...
- ✅ 实际的 Agent 定义在 `toolchain/agents/aw-kernel/`
...
- **变更**:
  - 移动 7 个配置/文档文件到 `toolchain/docs/aw-kernel/`
  - 移动资源目录到 `toolchain/assets/aw-kernel/templates/`
  - `toolchain/agents/aw-kernel/` 现在只包含 Agent 定义文件（12个）
- **好处**: 职责单一，易于维护和查找
