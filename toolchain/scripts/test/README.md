...
- `agents_first_wave_contract_smoke.py`：在隔离 install root 和生成的 `.aw` fixture 上，证明 `agents` first-wave payload 可读取，并且 `harness -> repo-status -> repo-whats-next -> init-worktrack -> dispatch` 的最小 contract 路径成立；它属于 deploy / contract smoke，不宣称真实 `SubAgent` dispatch 或 autonomy loop 已经落地
