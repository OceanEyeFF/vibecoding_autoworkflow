...
- `agents_first_wave_smoke.py`：在隔离 install root 和生成的 `.aw` fixture 上，证明 `agents` first-wave payload 可读取，并且 `harness -> repo-status -> repo-whats-next -> init-worktrack -> dispatch` 的最小 I/O 路径成立
