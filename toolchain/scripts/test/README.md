...
- `closeout_acceptance_gate.py`：按 closeout 顺序聚合 scope/spec/static/cache/test/smoke gates；其中 cache gate 会拒绝 `docs/`、`product/`、`toolchain/` 和 `tools/` 下的 Python / pytest 运行缓存，test gate 会运行 closeout、folder、path、semantic、agents adapter、deploy regression、Repo Analysis contract 回归测试、本地 `aw-installer` npm deploy package与根 package envelope 的 packlist dry-run、根 package publish dry-run及其 `prepublishOnly` guard、临时 `.tgz` help/version/diagnose/update dry-run/install/verify/update apply tarball smoke，以及 `adapter_deploy.py` / `harness_deploy.py` 的 deploy verify
