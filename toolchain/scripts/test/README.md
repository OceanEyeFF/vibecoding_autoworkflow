...
- `closeout_acceptance_gate.py`：按 closeout 顺序聚合 scope/spec/static/cache/test/smoke gates；其中 cache gate 会拒绝 `docs/`、`product/`、`toolchain/` 和 `tools/` 下的 Python / pytest 运行缓存，test gate 会运行 closeout、folder、path、semantic、agents adapter、Repo Analysis contract 回归测试、本地 npm deploy package 的 packlist dry-run 和临时 `.tgz` help/diagnose tarball smoke，以及 `adapter_deploy.py` / `harness_deploy.py` 的 deploy verify
