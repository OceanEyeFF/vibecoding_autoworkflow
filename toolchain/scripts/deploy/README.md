...
- `adapter_deploy.py`：保留为 repo-local Python reference / parity CLI，不再进入 `aw-installer` package runtime payload
...
- `harness_deploy.py` 当前只作为 repo-local Python reference 的薄包装入口存在；根目录 `package.json` 是 self-contained `aw-installer` package envelope，本地 package scaffold 只暴露 `aw-installer` bin，但不表示 npm release channel 已发布
