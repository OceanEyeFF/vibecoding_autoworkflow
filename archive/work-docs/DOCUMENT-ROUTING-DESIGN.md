...
| Agent | 文件 | 何时使用 | 预计耗时 |
|-------|------|---------|----------|
| **feature-shipper** | [feature-shipper.md](toolchain/agents/aw-kernel/feature-shipper.md) | 实现具体功能 | 1-2 小时 |
| **code-analyzer** | [code-analyzer.md](toolchain/agents/aw-kernel/code-analyzer.md) | 代码结构分析 | 10-30 分钟 |
| **code-debug-expert** | [code-debug-expert.md](toolchain/agents/aw-kernel/code-debug-expert.md) | 问题定位与修复 | 30-60 分钟 |
| **requirement-refiner** | [requirement-refiner.md](toolchain/agents/aw-kernel/requirement-refiner.md) | 需求澄清细化 | 15-30 分钟 |
| **code-project-cleaner** | [code-project-cleaner.md](toolchain/agents/aw-kernel/code-project-cleaner.md) | 代码清理重构 | 30-60 分钟 |
| **system-log-analyzer** | [system-log-analyzer.md](toolchain/agents/aw-kernel/system-log-analyzer.md) | 日志分析诊断 | 20-40 分钟 |
| **knowledge-researcher** | [knowledge-researcher.md](toolchain/agents/aw-kernel/knowledge-researcher.md) | 技术资料研究 | 30-60 分钟 |

**Skill（工作流）**：
- **autodev**：[autodev/SKILL.md](toolchain/skills/aw-kernel/autodev/SKILL.md) - 完整开发流程（需求 → 实现 → 交付）
...
开始任务
  ...
  │
  ├─ 需要使用单个 Agent？
  │    └─ 是 → 读 toolchain/agents/aw-kernel/{agent}.md（L1）
  │
  ├─ 遇到协作问题？
  │    └─ 是 → 读 GUIDE.md Part 3（L2，按需）
  │
  ├─ 需要理解设计思路？
...
|---------|------|---------|---------|--------|
| **新功能开发** | L1 | [WORKFLOW.md](WORKFLOW.md) | [autodev/SKILL.md](toolchain/skills/aw-kernel/autodev/SKILL.md) | 需要完整流程时 |
| **单个功能实现** | L1 | [feature-shipper.md](toolchain/agents/aw-kernel/feature-shipper.md) | - | 需求已明确时 |
| **代码分析** | L1 | [code-analyzer.md](toolchain/agents/aw-kernel/code-analyzer.md) | - | 需要架构评估时 |
| **调试问题** | L1 | [code-debug-expert.md](toolchain/agents/aw-kernel/code-debug-expert.md) | [system-log-analyzer.md](toolchain/agents/aw-kernel/system-log-analyzer.md) | 遇到 Bug 时 |
| **需求澄清** | L1 | [requirement-refiner.md](toolchain/agents/aw-kernel/requirement-refiner.md) | - | 需求模糊时 |
| **代码清理** | L1 | [code-project-cleaner.md](toolchain/agents/aw-kernel/code-project-cleaner.md) | - | 需要重构时 |
| **资料研究** | L1 | [knowledge-researcher.md](toolchain/agents/aw-kernel/knowledge-researcher.md) | - | 需要技术调研时 |
...
| 文档 | 内容 | 长度 | 何时读 |
|------|------|------|--------|
| **GUIDE.md Part 1** | 项目定位、核心规则、Agent 清单、路由指南 | ≤ 100 行 | ✅ **每次新对话立即读** |
...
| 文档 | 内容 | 何时读 | 为什么延迟 |
|------|------|--------|-----------|
| **GUIDE.md Part 3** | 详细协作规范、禁区说明 | 遇到协作问题时 | 信息密度高，非必需 |
...
```
1. ✅ 读 GUIDE.md Part 1（已读）
2. ✅ 参考 Agent 清单 → 选择 feature-shipper
3. ✅ 读 toolchain/agents/aw-kernel/feature-shipper.md（L1）
4. ✅ 执行任务
...
**选项**：
- A: 删除（内容合并到 GUIDE.md Part 2）
- B: 保留（但标记为 L2，给人看的）

浮浮酱推荐删除（功能被 GUIDE.md Part 2 替代）
