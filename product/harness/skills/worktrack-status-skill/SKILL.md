...
- 本技能是控制回路的传感器层，负责状态估计。算子选择、任务分派或动作执行的行为禁止出现在本技能的输出中——这些属于 Decide/Dispatch/Implement 阶段的职责。
- 本技能的唯一合法行为是观察工作追踪状态。选择工作追踪下一步动作的行为禁止发生——那是 `Decide` (`schedule-worktrack-skill`) 的职责。
- 本技能的唯一合法输出是为 `Harness` 产出稳定的字段包。扩展成规划、编码或执行分派的行为必须被阻断。
- 对 `.aw/repo/*` 的唯一合法操作是作为次要边界证据读取。重写 `.aw/repo/*` 的行为必须被阻断。
- 优先使用标准工作追踪产物，而不是工作追踪本地部署副本或模板。
- 当当前问题很狭窄时，唯一合法的行为是保持限定范围。扩展成完整工作追踪重新发现的行为必须被阻断。
- 选择下一步动作是 `Decide` (`schedule-worktrack-skill`) 的职责。从本技能选择下一步动作的行为必须被阻断。
- 分派执行是 `Dispatch` (`dispatch-skills`) 的职责。从本技能分派执行的行为必须被阻断。
- 输出 gate 判定是 `Judge` (`gate-skill`) 的职责。从本技能输出 gate 判定的行为必须被阻断。
- 状态估计的输出只能是结构化状态观察。决策行为禁止出现在状态估计的输出中——状态估计是决策的输入，不是决策本身。
- 状态报告的时效性标注必须反映实际状态。将过期状态标注为新鲜状态的行为必须被阻断。
- 对 `Worktrack Contract` 和 `Plan` 的唯一合法操作是读取消费。覆盖这些产物的行为必须被阻断。
- 变更 `Harness 控制状态` 的行为必须被阻断。本技能仅产出状态估计供监督器决策使用。
