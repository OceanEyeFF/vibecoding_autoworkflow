...
- 本技能是控制回路的算子选择层，基于状态估计结果选择下一步动作。状态估计或动作执行的行为禁止出现在本技能的输出中——状态估计属于 Observe 阶段，动作执行属于 Dispatch/Implement 阶段。
- 本技能的范围边界只能是当前 `工作追踪约定` 所定义的范围。超出约定的行为禁止发生。
- 执行已选下一步动作是 Dispatch/Implement 阶段的职责，直接分派下游工作是 `dispatch-skills` 的职责。从本技能执行或分派的行为必须被阻断。
...
- 选择执行者或专用技能绑定属于 `分派技能` (`dispatch-skills`) 的职责范围。该行为禁止出现在本技能的职责边界内。
- 验收标准、排除目标或恢复策略的唯一合法来源是已被批准的上游产物（Goal Charter、`Worktrack Contract`）。凭空发明的行为必须被阻断。
- 验收标准在本技能中的唯一合法角色是规划约束和覆盖目标。验收标准的验证或通过判定禁止在本技能中发生——那是 Verify/Judge 阶段的职责。
...
- 当限定范围刷新已经足够时，唯一合法行为是执行限定范围刷新。重写整个队列的行为必须被阻断。
- 当不存在安全的下一步动作时，唯一合法行为是明确返回该结论及阻塞原因。隐藏歧义的行为必须被阻断。
- 变更 `Harness 控制状态` 和输出关卡判定结果的行为必须从本技能被阻断。前者是监督器的职责，后者是 `gate-skill` 的职责。
- 跨越 `RepoScope` 与 `WorktrackScope` 的唯一合法条件是拥有经批准的权限边界。未经批准的跨 Scope 切换必须被阻断。
...
- 变更控制状态前唯一合法的前提是已显式暴露请求中的状态迁移。因下一步看起来明显而直接变更控制状态的行为必须被阻断。
