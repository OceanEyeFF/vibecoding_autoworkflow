...
- 本技能是控制回路的分派执行层；负责绑定技能、打包任务并分派执行。算子选择或裁决判定的行为禁止出现在本技能的输出中——算子选择属于 Decide 阶段，裁决判定属于 Judge 阶段。
- 工作项的唯一合法边界是当前 `工作追踪约定` 与 `计划/任务队列`。超出此边界的行为必须被阻断。
- 当前下一步动作的唯一合法来源是规划阶段（`schedule-worktrack-skill`）已经给出的已选工作项。在本技能内重新选择、重排或发明下一动作的行为必须被阻断。
- 被分派任务的唯一合法验收依据是调度阶段已经建立好的验收标准切片与验收对齐结果。脱离该依据的行为必须被阻断。
- 当调度包缺失时，唯一合法行为是返回调度阶段。创建新的权威 `分派任务简报` 或 `分派信息包` 的行为必须被阻断。
- "没有专用技能可用"本身不构成阻塞状态。该情况下应走通用回退路径（`generic-worker-skill` 或一次性任务专用指令），而不是停止流程。
- 在没有专用技能时，唯一合法行为是在原任务边界内给出本轮任务专用指令与执行载体选择。扩大任务边界或把一次性指令写成新的 canonical skill 的行为必须被阻断。
- 当限定范围信息包已经足够时，唯一合法传递内容是限定范围信息包。传入完整代码仓库上下文的行为必须被阻断。
- 回退执行载体的唯一合法行为是在已有验收标准、排除目标和验证要求的约束内执行。重定义这些约束的行为必须被阻断。
- 当宿主运行时支持真实 SubAgent 委派且权限边界允许时，默认路径是分派 SubAgent。当前载体内执行不能成为首选路径，仅允许在明确回退条件下使用。
- 当过大的调度包跨越多个队列项、多个验收切片或开放式"完成全部"批次时，唯一合法行为是返回调度阶段并携带过大原因。静默规范成执行的行为必须被阻断。
- 一旦包已不再适合一轮限定范围执行，继续分派的行为必须被阻断；唯一合法行为是带着过大原因返回调度阶段。
- 仅当调度包已经携带显式的原子性理由时，通过合并实现、清理和验证工作来放宽首个面向执行切片的行为才合法；否则必须保持原切片粒度或返回调度阶段。
- 仅当宿主运行时真的创建了委派载体时，声称使用了委派式 `子代理` 才合法；否则必须显式报告未委派原因（`runtime fallback`、`permission blocked` 或 `dispatch package unsafe`）。
- 如果没有使用委派式 `子代理`，必须在结果中给出明确阻断原因：`runtime fallback`、`permission blocked` 或 `dispatch package unsafe`。
- 变更 `Harness 控制状态` 和输出关卡判定结果的行为必须从本技能被阻断。前者是监督器的职责，后者是 `gate-skill` 的职责。
- 选择理由、执行结果与证据的唯一合法呈现形式是结构化字段。压缩成模糊摘要的行为必须被阻断。
- 仅当当前工作项已从活动 `Plan/Task Queue` 中选出后，从 `WorktrackScope` 发起分派才合法；否则必须返回调度阶段。
- 当没有命中完全匹配的专用技能时，唯一合法行为是先把任务压缩成受约束的执行包，再选择专用技能、通用 SubAgent 或明确的当前载体回退。直接吸收执行责任的行为必须被阻断。
