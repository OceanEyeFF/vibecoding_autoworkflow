...
这个技能会消费一个已经选定的当前工作项，以及 `schedule-worktrack-skill` 为其准备的限定范围分派交接包；当存在明显匹配的专用技能时，选择最合适的那个，否则回退到通用任务完成执行载体。没有现成专用技能不是阻塞条件：分派结果可以给出一次性、限定范围的任务专用执行指令，并绑定到通用 `SubAgent` 或明确的当前载体回退，但不得把这类一次性指令伪装成新的永久技能。
...
9. 如果没有合适专用 skill，就使用同一份限定范围任务/信息约定，通过通用任务完成 `SubAgent` 分派。
