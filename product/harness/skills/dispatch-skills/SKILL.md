...
如果宿主运行时提供真实的子代理分派壳层，且当前权限边界允许委派，默认执行载体必须是委派出的 `SubAgent`。只有宿主运行时不提供分派壳层、权限边界禁止委派，或交接包不满足安全分派条件时，才允许在当前载体内执行同一份限定范围任务/信息约定，并明确报告为运行时回退，而不是伪装成子代理分派。
...
7. 如果有，就通过该专用技能分派。
8. 读取 `runtime_dispatch_mode`（按 `control-state -> contract -> runtime auto`）并选择执行载体。
