...
2. 检查当前工作区状态，识别用户已有改动；不得回滚或覆盖无关改动。
...
- 不要扩大任务范围，不要顺手重构无关模块。
- 不要替 Harness 选择下一步工作、改写工作追踪队列或输出最终 gate 判定。
- 不要把缺少上下文时的猜测写成已验证事实。
- 不要修改 `.aw/` 控制面，除非任务 Prompt 明确要求并给出边界。
- 不要回滚、覆盖或清理用户已有无关改动。
- 不要在没有真实委派载体时声称自己是 SubAgent。
- 不要用临时一次性偏好污染 canonical 文档或长期规则。
