...
5. 将项目分开：
   ...
   - 推迟或仍未验证的项目
   - 收尾后仍开放的代码仓库级风险
6. 将已关闭 worktrack 的条目（worktrack_id / status / node_type / scope / merge_commit / validation）追加到 `.aw/repo/worktrack-backlog.md`（若 backlog 不存在则创建），条目按时间倒序排列。此步骤对每个已验证关闭的 worktrack 无条件执行。
