...
5. 将项目分开：
   ...
   - 推迟或仍未验证的项目
   - 收尾后仍开放的代码仓库级风险
6. 如果存在活跃 Milestone，检查已关闭 worktrack 是否在 Milestone 的 `worktrack_list` 中；若在列表中，标记 Milestone progress counter 可能需要更新（由 milestone-status-skill 在下一轮 Observe 中执行）。
