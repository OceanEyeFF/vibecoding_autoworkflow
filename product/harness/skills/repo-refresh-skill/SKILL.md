...
3. 从 close-worktrack 交接读取 `baseline_branch`、PR target、merge target 与 checkpoint 基准；这些值必须来自原始 `Worktrack Contract.baseline_branch`，不得从当前分支名或写死默认分支名推断。
4. 根据 `代码仓库目标/章程`、当前 `代码仓库快照/状态` 和已验证收尾证据刷新代码仓库级评估。
5. 将项目分开：
   - 已验证回写候选
   - 推迟或仍未验证的项目
   - 收尾后仍开放的代码仓库级风险
6. 在一轮限定范围代码仓库刷新后停止，并返回一份固定格式的 `代码仓库刷新报告` 加一份 `已验证回写交接`。
...
- 不要把当前分支或本地默认分支当作 repo baseline；代码仓库刷新使用 close-worktrack 交接中来自 `Worktrack Contract.baseline_branch` 的 baseline。
