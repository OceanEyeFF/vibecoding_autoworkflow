...
5. 将项目分开：
   ...
   - 推迟或仍未验证的项目
   - 收尾后仍开放的代码仓库级风险
6. 如果存在活跃 Milestone，检查已关闭 worktrack 是否在 Milestone 的 `worktrack_list` 中；若在列表中，标记 Milestone progress counter 需要由 milestone-status-skill 在下一轮 Observe 中更新。将已关闭 worktrack 的条目（worktrack_id / status / node_type / scope / merge_commit / validation）追加到 `.aw/repo/worktrack-backlog.md`（若 backlog 不存在则创建），条目按时间倒序排列。
7. 在一轮限定范围代码仓库刷新后停止，并返回一份固定格式的 `代码仓库刷新报告` 加一份 `已验证回写交接`。
