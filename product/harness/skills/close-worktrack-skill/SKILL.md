...
优先级：`Worktrack Contract` 中显式填写的 `baseline_form`、`merge_required`、`if_interrupted_strategy` 优先；下表只作为节点类型默认值。若实际 checkpoint 与 contract policy 不一致，必须在代码仓库刷新交接中标记 `checkpoint_policy_match: no` 并请求审批。
