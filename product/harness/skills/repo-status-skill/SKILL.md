...
- 如果 `latest_observed_checkpoint` 记录的 hash 在当前 git history 中不可达（如曾被 `git reset`），应标记 `baseline_gap_risk: high` 并强制重新刷新（此时 hash 必然与当前 HEAD 不一致，上一条规则已要求重新刷新，本条为补充风险标记）。
