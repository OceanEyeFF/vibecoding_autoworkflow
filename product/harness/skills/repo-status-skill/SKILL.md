...
- 本技能是控制回路的传感器层，负责状态估计；状态估计的输出仅限于传感器数据；算子选择或动作执行的行为禁止出现在本技能的输出中。
- 唯一合法行为是观察代码仓库状态；选择下一步动作的行为必须标记为超出本技能权限。
- 唯一合法行为是为 `Harness` 产出稳定的字段包；扩展成规划、编码或执行分派的行为必须返回 blocked。
- 对 `.aw/worktrack/*` 的唯一合法行为是将其读取为边界证据；重写 `.aw/worktrack/*` 的行为必须标记为超出本技能权限。
- 优先使用标准代码仓库产物，而不是代码仓库本地部署副本或模板。
- 唯一合法行为是保持在限定范围内观察；扩展成完整代码仓库重新发现的行为必须返回 blocked。
- 仅当 `latest_observed_checkpoint` 已存在且其记录的 hash 在当前 git history 中可达时，才可跳过刷新；首次启动时 `latest_observed_checkpoint` 缺失，必须执行完整状态估计。
...
- `milestone_status`：当前活跃 Milestone 的状态（planned / active / completed / blocked / N/A）
- `主线状态`
- `活动分支`
- `治理信号`
- `已知风险`
