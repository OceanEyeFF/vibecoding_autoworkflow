...
如果运行时不能真实并行委派，必须显式记录 `runtime fallback`、`permission blocked` 或 `dispatch package unsafe`，并在同一份审查证据中保留四个 lane 的覆盖状态；不得把当前载体内的串行分析伪装成已创建四个 SubAgent。
...
- 当运行时支持真实 SubAgent 委派且权限边界允许时，必须并行分派四个 review SubAgent；不得降级成单个泛化 review。
- 四个 review lane 必须分别保留输入范围、输出摘要、覆盖状态和缺失证据；不得把四路结果压成无法追溯的一段综合意见。
