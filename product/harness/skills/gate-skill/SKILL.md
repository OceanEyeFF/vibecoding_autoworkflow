...
- 重复性上下文（如 worktrack contract 摘要）引用文件路径，不要内联全文
...
- 不要把 review evidence 的五类覆盖缺口吞并到笼统的 implementation-gate 通过结论里；性能、架构、安全、质量、测试五类 review 面必须被接收、标为不适用并说明理由，或暴露为缺失证据。
