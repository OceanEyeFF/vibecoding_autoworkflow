...
- 本技能是控制回路的裁决层；负责基于证据做 gate 判定。执行修复、重跑实现工作或变更 Harness 控制状态的行为必须从本技能被阻断——修复和实现属于 Implement 阶段，控制状态变更属于监督器职责。
- 执行修复、重跑实现工作或重新收集代码仓库新鲜状态的行为必须从本技能被阻断。这些分别属于 Implement 和 Verify 阶段的职责。
- 缺失证据的唯一合法处理方式是显式暴露缺口。将缺失证据当成隐式成功的行为禁止发生。
...
- 对 `工作追踪约定`、验收标准或排除目标的唯一合法操作是读取消费。改写这些产物的行为必须被阻断。
- 从本技能直接跳进恢复、合并、收尾或代码仓库刷新的行为必须被阻断。本技能只给出判定结果与允许的下一路由，后续动作由相应阶段算子（Recover/Close/RepoScope）执行。
- 变更 `Harness 控制状态` 的行为必须从本技能被阻断。唯一合法行为是返回一个供监督器使用的判定结果。
- 仅当当前证据边界明确依赖相邻系统时，扩展进相邻系统才合法；否则必须严格保持在当前证据边界内。
