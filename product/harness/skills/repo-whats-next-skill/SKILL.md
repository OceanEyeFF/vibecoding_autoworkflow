...
12. 检查活跃 Milestone 信号：如果存在活跃 Milestone 且其 `milestone_acceptance_verdict` 为 `blocked` 或 `proceed_blockers` 非空，应在 `继续阻塞项` 中反映 Milestone 阻塞状态；如果活跃 Milestone 的 `handback_required` 为 true，应考虑建议 handback 而非自动推进。
