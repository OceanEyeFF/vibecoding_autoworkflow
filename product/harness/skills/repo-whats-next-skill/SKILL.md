...
12. 执行 Milestone-First 判定：
    若 `active_milestone` 为空（无活跃 Milestone）：
      - 读取 milestone-backlog，检查是否存在满足激活条件的 planned milestone
      - 若存在：`suggested_milestone_action = "activate"`，输出 `suggested_milestone_id`
      - 若不存在：`suggested_milestone_action = "create"`，基于 Goal Charter 推理建议新 milestone
      - `suggested_next_scope = "RepoScope"`，绑定 `init-milestone-skill`
      - **禁止在此分支建议"进入工作追踪"**
...
- **Milestone-First**：无 active milestone 时，"进入工作追踪" 路由必须 blocked，必须优先建议创建或激活 milestone。
...
- `suggested_milestone_action`：activate / create / continue / closeout / none
