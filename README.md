...
推荐将 `.claude/agents/feature-shipper.md` 复制/软链到目标项目的 `.claude/agents/`，按需附带 `code-debug-expert` / `requirement-refiner` 等。
同时同步本仓库的 `.claude/skills/` 到目标项目（至少 `autoworkflow` 与 `git-workflow`），以便 subagent 通过 `skills:` 加载可复用流程能力。
然后在 Claude Code 里选择 `feature-shipper`，它会强制先打磨 spec / DoD，再坚持 gate（测试全绿）闭环推进。
