...
`aw-installer` 是已批准的 unscoped npm package identity，也是当前仓库内分发入口和 CLI bin。`aw-installer@0.4.0-rc.1` 已发布为 npm registry RC；当前 registry 的 `next` 和 `latest` 都指向这个唯一 RC 版本。P0-020 完成 registry smoke 和文档主路径切换前，外部试用应显式使用 `aw-installer@next` 或维护者提供的本地 `.tgz`。复制粘贴入口见 [`aw-installer Public Quickstart Prompts`](./docs/project-maintenance/deploy/aw-installer-public-quickstart-prompts.md)。
...
这里的 `aw-installer@next` 是当前 RC 选择器；裸 `npx aw-installer` 技术上会解析到同一个唯一 RC，但 P0-020 smoke 通过前不把它作为文档主路径。`diagnose` 和 `verify` 是只读检查；`install` 是显式写入当前 payload 的底层命令；`update` 默认只输出 dry-run plan。推荐写入路径是在确认 plan 后运行 `update --yes`，它会按 `prune --all -> check_paths_exist -> install -> verify` 写入目标仓库的 `.agents/skills`。完整入口合同见 [`Distribution Entrypoint Contract`](./docs/project-maintenance/deploy/distribution-entrypoint-contract.md)，维护者验证与本地 tarball smoke 见 [`Deploy Runbook`](./docs/project-maintenance/deploy/deploy-runbook.md)。
...
当前 RC 最短路径是在目标项目根目录显式运行 `next` selector：
...
P0-020 registry smoke 通过前，不要把裸 `npx aw-installer` 当作 primary path。当前外部试用可使用 `aw-installer@next`、维护者提供的本地 `.tgz` 或明确 checkout source；完整复制粘贴流程见 [`aw-installer Public Quickstart Prompts`](./docs/project-maintenance/deploy/aw-installer-public-quickstart-prompts.md)。
